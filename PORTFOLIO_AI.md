...
두 차례의 AI B2C 서비스 초기 멤버로 참여하며 0 to 1 단계에서의 설계 구현 검증을 반복해왔습니다. 이 과정에서 AI를 단순한 구현 도구가 아니라 설계 단계부터 함께 의논하고 판단을 보조하는 파트너로 활용하며 의사결정 속도를 크게 높였습니다. 그 결과 개인의 경험과 지식 범위를 넘어서는 시도를 빠르게 검증할 수 있었고 더 많은 실험과 개선을 짧은 주기로 반복할 수 있었습니다. 이러한 경험을 통해 AI와 함께 일하는 방식이 서비스 초기의 불확실성을 줄이고 실행력을 높이는 데 실질적인 도움이 된다는 것을 체감했습니다. 코코네처럼 방대한 유저 데이터와 서비스 맥락을 가진 환경에서는 이러한 의사결정 방식이 더 큰 효과를 낼 수 있다고 생각하며 초기 실험을 빠르게 현실로 만드는 역할로 기여하고 싶습니다.
...
### 앱·호출 구조 요약
- **화면**: 스플래시 → 로그인 → 홈(사주/전문가/채팅/스토어). 사주: 정통·오늘·신년 스트리밍; 전문가: 도사 목록 → 채팅; 스토어: 코인 충전(IAP/Mock).
- **AI 호출**: 클라이언트 → Edge Function → OpenAI(스트리밍) → SSE → 화면 갱신. 채팅은 시스템(도사+사주 스냅샷) + 유저(요약+질문) 조합 후 DB 저장·요약 갱신.
